...
def _build_tracker(ctx: PipelineContext) -> ObjectTracker:
...
def run(ctx: PipelineContext) -> PipelineStageStatus:
    ...
    logger.info(
        "[%s] Processing ALL %d frames (detection frames: %d, interpolation frames: %d)",
        STAGE, total_frames, len(detection_map), total_frames - len(detection_map)
    )
